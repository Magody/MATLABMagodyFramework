function [reward, new_state, finish] = getRewardCNNGridWorld(state, action_selected, context) 
    ...
    if map_actions(action_selected).collision
        ...
        matrix(map_actions(action_selected).position(1), map_actions(action_selected).position(2)) = 0;
    ...
    shape_input = context('shape_input');
    new_state =reshape(matrix(:), shape_input);
